...
export default function MyProgram() {
  ...
  const programWithVideos = myProgram.map(prog => ({
    ...prog,
    video: mediaLibrary.find(v => v.id === prog.media_library_id)
  })).filter(p => p.video).sort((a, b) => a.order - b.order);

  const completedCount = programWithVideos.filter(p => p.completed).length;
                    ...
                    <div className="flex-1">
                      ...
                      {item.video.topics && item.video.topics.length > 0 && (
                        <div className="flex flex-wrap gap-1 mb-3">
                          {item.video.topics.map((topic, idx) => (
