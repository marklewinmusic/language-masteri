...
import { ArrowLeft, Upload, Play, Pause, Plus, X } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
import { Button } from "@/components/ui/button";
import { toast } from "sonner";

export default function SpeakAudio() {
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full space-y-6">
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
            ...
            {transcriptFile && transcriptFile.length > 0 && (
              <div className="space-y-2">
                <p className="text-white/40 text-xs uppercase tracking-wider mb-3">Words to Practice</p>
            ...
            {(!transcriptFile || transcriptFile.length === 0) && !showUploadForm && (
              <p className="text-white/40 text-sm text-center py-4">No transcript added yet. Click "Add Transcript" to get started.</p>
            )}
          </div>
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl p-4">
            <p className="text-white/60 text-sm">
              💡 <strong>Tip:</strong> Play the audio and practice speaking the words shown above. Focus on pronunciation and fluency.
            </p>
          </div>
        </motion.div>
      </div>
